...
const handleSingleLogout = handleSAMLMessage.bind(null, [
  "LogoutRequest",
  "LogoutResponse",
]);
const handleAllMessages = handleSAMLMessage.bind(null, [
  "Response",
  "LogoutRequest",
  "LogoutResponse",
]);
  ...
  try {
    ...
    switch (messageType) {
      case "Response":
        ...
        opt.wantSignedAssertion &&
          (await verifySAMLSignature(root.Assertion, opt.verifyKeys));

        /* Exctract NameID, NameIDFormat and check the SubjectConfirmation if present */
        node = root.Assertion.Subject.NameID
          ? root.Assertion.Subject.NameID
        ...
        const authnStatement = parseAuthnStatement(
          root.Assertion.AuthnStatement,
        );
...
function processSAMLMessageHeader(r, opt, root) {
  ...
  if (issueInstantDate > currentTime) {
    throw Error(
      `IssueInstant is in the future. Check clock skew of SP and IdP`,
    );
  }
  ...
  if (inResponseTo) {
    ...
    if (r.variables.saml_request_redeemed != "1") {
      throw Error(
        `InResponseTo attribute value "${inResponseTo}" ` +
  ...
  if (destination && destination !== opt.spServiceURL) {
    throw Error(
      `The SAML Destination "${destination}" does not match ` +
...
function checkIssuer(root, idpEntityId) {
  ...
  if (issuer && issuer !== idpEntityId) {
    throw Error(
      `Issuer "${issuer}" does not match IdP EntityID "${idpEntityId}"`,
    );
  }
}
...
function checkSubjectConfirmation(root) {
  ...
  if (root.$attr$Method === "urn:oasis:names:tc:SAML:2.0:cm:bearer") {
    ...
    if (!root) {
      throw new Error(
        "SubjectConfirmationData element is missing in the " +
          "SubjectConfirmation",
      );
    }

    const now = new Date();
    const notOnOrAfter = root.$attr$NotOnOrAfter
      ? new Date(root.$attr$NotOnOrAfter)
      : null;
    if (notOnOrAfter && notOnOrAfter < now) {
      throw new Error(
        `The Subject has expired. Current time is ${now} ` +
...
function checkConditions(root, spEntityId) {
  ...
  const now = new Date();
  const notBefore = root.$attr$NotBefore
    ? new Date(root.$attr$NotBefore)
    : null;
  const notOnOrAfter = root.$attr$NotOnOrAfter
    ? new Date(root.$attr$NotOnOrAfter)
    : null;
  ...
  if (notBefore && notBefore > now) {
    throw Error(
      `The Assertion is not yet valid. Current time is ${now} and ` +
  ...
  if (notOnOrAfter && notOnOrAfter < now) {
    throw Error(
      `The Assertion has expired. Current time is ${now} and ` +
  ...
  if (root.AudienceRestriction && root.AudienceRestriction.Audience) {
    ...
    if (!spFound) {
      throw Error(
        "The Assertion is not intended for this Service Provider. " +
...
function parseAuthnStatement(root, maxAuthenticationAge) {
  ...
  if (maxAuthenticationAge) {
    ...
    const now = new Date();
    if (
      now.getTime() - authnInstantDate.getTime() >
      maxAuthenticationAge * 1000
    ) {
      return false;
    }
  }
  ...
  const sessionNotOnOrAfter = root.$attr$SessionNotOnOrAfter
    ? new Date(root.$attr$SessionNotOnOrAfter)
    : null;
  if (sessionNotOnOrAfter && sessionNotOnOrAfter < now) {
    throw Error(
      `The Assertion Session has expired. Current time is ${now} and ` +
  ...
  if (!root.AuthnContextClassRef) {
    throw Error(
      "The AuthnContextClassRef element is missing in the AuthnContext",
    );
  }
...
function saveSAMLAttributes(r, root) {
  ...
  for (var attributeName in attrs) {
    if (attrs.hasOwnProperty(attributeName)) {
      ...
      if (
        attributeName.includes("http://") ||
        attributeName.includes("https://")
      ) {
...
function extractSAMLParameters(r) {
  try {
    ...
    throw Error(
      `Failed to extract SAMLRequest or SAMLResponse parameter ` +
...
function getPayload(r) {
  switch (r.method) {
    case "GET":
      return r.variables.arg_SAMLResponse || r.variables.arg_SAMLRequest
        ? r.variables.args
        : null;
    case "POST":
      return r.headersIn["Content-Type"] ===
        "application/x-www-form-urlencoded" && r.requestText.length
        ? r.requestText
        : null;
    default:
      return null;
...
function parsePayload(payload, method) {
  ...
  let samlResponse = Buffer.from(
    decodeURIComponent(params.SAMLResponse || params.SAMLRequest),
    "base64",
  );
...
function setSessionCookie(r) {
  ...
  r.headersOut[
    "Set-Cookie"
  ] = `auth_token=${authToken}; ${r.variables.saml_cookie_flags}`;
...
function checkReplayAttack(r, id, type) {
  ...
  if (r.variables.saml_response_redeemed === "1") {
    throw Error(
      `An attempt to reuse a ${type} ID was detected: ` +
  ...
  try {
    ...
    switch (messageType) {
      ...
      case "LogoutResponse":
        ...
        opt.statusCode = getLogoutStatusCode(
          r.variables.saml_name_id,
          opt.nameID,
        );
        break;
...
function validateMessageType(messageType) {
  ...
  if (!allowedMessageTypes.includes(messageType)) {
    throw new Error(
      `Invalid messageType: ${messageType}. ` +
...
async function createSAMLMessage(opt, id, messageType) {
  ...
  const assertionConsumerServiceURL =
    handlerResult.assertionConsumerServiceURL || "";
  ...
  try {
    ...
    throw Error(
      `Failed to create ${messageType} from XML template: ${e.message}`,
    );
  }
...
function dispatchResponse(
  ...
  if (isPost) {
    ...
    return r.return(
      200,
      postFormTemplate(encodedXml, idpServiceUrl, relayState, postParam),
    );
  } else {
    ...
    const url =
      `${idpServiceUrl}?${postParam}=${encodeURIComponent(encodedXml)}` +
...
async function verifySAMLSignature(root, keyDataArray) {
  ...
  for (let i = 0; i < keyDataArray.length; i++) {
    try {
      ...
      const signatureResult = await signatureSAML(
        rootSignature,
        keyDataArray[i],
      );
  ...
  throw Error(
    `Error verifying ${type} message signature: ${errors.join(", ")}`,
  );
}
...
async function digestSAML(signature, produce) {
  ...
  if (URI != `#${ID}`) {
    throw Error(
      `signed reference URI ${URI} does not point to the parent ${ID}`,
    );
  }
  ...
  if (
    transformAlgs[0] != "http://www.w3.org/2000/09/xmldsig#enveloped-signature"
  ) {
...
function keyPem2Der(pem, type) {
  ...
  const pemContents = pemJoined.substring(
    pemHeader.length,
...
async function signatureSAML(signature, key_data, produce) {
  ...
  const withComments =
    signedInfo.CanonicalizationMethod.$attr$Algorithm.slice(39) ==
    "WithComments";
...
async function decryptSAML(root, key_data) {
  ...
  const keyAlg =
    root.EncryptedData.KeyInfo.EncryptedKey.EncryptionMethod.$attr$Algorithm;
  ...
  const encryptedKeyNode =
    root.EncryptedData.KeyInfo.EncryptedKey.CipherData.CipherValue;
...
function parseConfigurationOptions(r, messageType) {
  ...
  if (messageType === "Response") {
    opt.wantSignedResponse = validateTrueOrFalse(
      "saml_sp_want_signed_response",
    );
    opt.wantSignedAssertion = validateTrueOrFalse(
      "saml_sp_want_signed_assertion",
    );
    opt.wantEncryptedAssertion = validateTrueOrFalse(
      "saml_sp_want_encrypted_assertion",
    );
  }
  ...
  if (messageType === "LogoutResponse" || messageType === "LogoutRequest") {
    ...
    if (!opt.isSLODisabled) {
      ...
      opt.logoutResponseURL = validateUrlOrUrn(
        "saml_idp_slo_response_url",
        true,
      );
  ...
  if (opt.wantSignedResponse || opt.wantSignedAssertion) {
    opt.verifyKeys = readKeysFromFile(
      r.variables.saml_idp_verification_certificate,
    );
  }
  ...
  function validateTrueOrFalse(name) {
    ...
    if (value !== "true" && value !== "false") {
      throw Error(
        `${prefix} Invalid "${name}": "${value}", must be "true" or "false".`,
      );
    }
...
function isUrlOrUrn(str) {
  const urlRegEx =
    /^((?:(?:https?):)\/\/)?((?:(?:[^:@]+(?::[^:@]+)?|[^:@]+@[^:@]+)(?::\d+)?)|(?:\[[a-fA-F0-9:]+]))(\/(?:[^?#]*))?(\\?(?:[^#]*))?(#(?:.*))?$/;
  const urnRegEx =
    /^urn:[a-z0-9][a-z0-9-.]{1,31}:[a-z0-9()+,\-.:=@;$_!*'%/?#]+$/i;
...
function readKeysFromFile(keyFile) {
  try {
    ...
    const regex =
      /-----BEGIN (PUBLIC|PRIVATE) KEY-----[\s\S]*?-----END (PUBLIC|PRIVATE) KEY-----/g;
    ...
    throw Error(
      `Failed to read private or public key from file "${keyFile}": ${e.message}`,
    );
  }
}
